...
export interface AdminSetupEnvUpdate {
  setupPassword: string;
  requiredEmail: string;
}

export async function updateAdminSetupEnv(input: AdminSetupEnvUpdate): Promise<{ filePath: string }> {
  const normalizedEmail = input.requiredEmail.trim().toLowerCase();
  const normalizedPassword = input.setupPassword;

  const nextValues = new Map<string, string>([
    ["BACKEND_ADMIN_SETUP_PASSWORD", serializeEnvValue(normalizedPassword)],
    ["BACKEND_ADMIN_REQUIRED_EMAIL", serializeEnvValue(normalizedEmail)],
  ]);
